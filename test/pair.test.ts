...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => new Pair(new TokenAmount(USDC, '100'), new TokenAmount(ETH, '100'))).toThrow(
        'CHAIN_IDS'
      )
    })
  })
  ...
  describe('#reserveOf', () => {
    ...
    it('throws if not in the pair', () => {
      expect(() =>
        new Pair(new TokenAmount(DAI, '101'), new TokenAmount(USDC, '100')).reserveOf(ETH)
      ).toThrow('TOKEN')
    })
  })
  ...
  describe('#involvesToken', () => {
    ...
    expect(
      new Pair(new TokenAmount(USDC, '100'), new TokenAmount(DAI, '100')).involvesToken(ETH)
    ).toEqual(false)
  })
})
